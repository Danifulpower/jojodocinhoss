...
const PRODUCT_SHEETS = [
    ...
    { name: 'pronta_entrega', range: 'pronta_entrega!A2:E' }, // Produto,Preço,Quantidade,Pedido,Imagem
];
 ...
 */
/**
 * Processa a aba 'pronta_entrega'
 * Espera a seguinte ordem: Produto, Preço, Quantidade, Pedido, Imagem
 * Colunas: A=Produto, B=Preço, C=Quantidade, D=Pedido, E=Imagem
 */
function processProntaEntrega(values) {
    ...
    values.forEach((row, index) => {
        if (row.length >= 4) {
            const [produto, precoStr, quantidadeStr, pedidoStr, imagem] = row;
            const preco = parseFloat(precoStr.replace(',', '.'));
            ...
            // Só adiciona se houver quantidade disponível
            if (disponivel > 0) {
                const item = {
                    nome: produto,
                    preco: preco,
                    ...
                    imagem: imagem || 'img/placeholder.jpg',
                    rowIndex: index + 2 // +2 porque começa na linha 2 (A2) da planilha
                };
                prontaEntrega.push(item);
            }
        }
    });
...
/**
 * Atualiza a coluna PEDIDO na aba pronta_entrega
 * @param {number} rowIndex - Índice da linha na planilha (ex: 2 para A2)
 * @param {number} novoPedido - Novo valor para a coluna PEDIDO
 */
async function updateProntaEntregaPedido(rowIndex, novoPedido) {
    if (!gisInited) {
        throw new Error('Autenticação necessária. Por favor, faça login.');
    }

    const range = `pronta_entrega!D${rowIndex}`; // Coluna D = PEDIDO
    ...
    try {
        ...
        console.log(`Pedido atualizado na linha ${rowIndex}: ${novoPedido}`);
        return true;
    } catch (err) {
        console.error('Erro ao atualizar pedido:', err);
        throw new Error('Falha ao atualizar estoque. Detalhes: ' + (err.result?.error?.message || err.message));
    }
}
...
window.updateProntaEntregaPedido = updateProntaEntregaPedido;

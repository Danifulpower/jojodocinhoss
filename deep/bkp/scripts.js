...
function showSection(sectionId) {
    ...
    if (sectionElement) {
        ...
        switch(sectionId) {
            ...
            case "ovos": loadOvosPascoa(); break;
            case "pronta_entrega": loadProntaEntrega(); break;
    ...
    container.innerHTML = produtos.prontaEntrega.map((produto, index) => {
        ...
        return `
            <div class="card product-card">
                ...
                <p class="product-description">Disponível: ${maxQuantidade} unidades</p>
                ...
                <div style="border-top: 1px solid #f0f0f0; padding-top: 1rem; margin-top: 1rem;">
                    ...
                    <button class="btn" onclick="addProntaEntregaToCart(${index})" ${quantidade > 0 ? '' : 'disabled'}>
...
function addProntaEntregaToCart(index) {
    ...
    if (quantidade > produto.quantidade) {
        alert(`Não há itens suficientes. Disponível: ${produto.quantidade}`);
        return;
    }
    ...
    carrinho.push({
        ...
        maxQuantidade: produto.quantidade // Guarda a quantidade máxima disponível
    });

    updateCartCount();
    alert(`${quantidade} unidade(s) de ${produto.nome} adicionada(s) ao carrinho!`);
    
    // Atualiza a quantidade disponível na lista
    produtos.prontaEntrega[index].quantidade -= quantidade;
    quantities[key] = 0;
    loadProntaEntrega();
}
...
function showFloatingImage(event, imagemSrc) {
    ...
    if (imagemSrc) {
        ...
        previewDiv.style.left = (event.pageX + 20) + "px";
        previewDiv.style.top = (event.pageY + 20) + "px";
    } else {
